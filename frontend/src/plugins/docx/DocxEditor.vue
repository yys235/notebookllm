...
import { SlashCommandExtension, SlashCommandMenu, defaultCommands, type SlashCommand } from './index'

// Props 定义 - 保持与 TiptapEditor 相同的接口
...
// 注册斜杠命令（预留接口）
function registerSlashCommand(command: SlashCommandItem) {
  slashCommands.value.push(command)
}
...
function showFloatingToolbar(x: number, y: number) {
  floatingToolbarPosition.value = { x, y }
...
// ========== 自定义斜杠命令 ==========
// 在默认命令基础上添加图片上传命令
const customSlashCommands: SlashCommand[] = [
  ...defaultCommands.filter(cmd => cmd.title !== '图片'), // 移除默认的图片命令
  {
    title: '图片',
    description: '上传或插入图片',
    icon: 'IMG',
    keywords: ['image', 'img', 'photo', 'upload'],
    action: () => {
      handleImageUpload()
    }
  }
...
<template>
  ...
  >
    <!-- 工具栏 -->
    <div v-if="editable" v-show="isEditorReady" class="docx-toolbar">
      <!-- 撤销/重做 -->
    ...
    <!-- 斜杠命令菜单 -->
    <SlashCommandMenu :editor="editor ?? null" />
    ...
    >
      <!-- 浮动工具栏内容将通过插槽或动态组件实现 -->
      <slot name="floating-toolbar"></slot>
    </div>
...
.docx-content :deep(.docx-editor:focus) {
  outline: none;
}

/* Placeholder */
.docx-content :deep(.docx-editor p.is-editor-empty:first-child::before) {
...
/* 图片样式 */
.docx-content :deep(.docx-editor img) {
  max-width: 100%;
  height: auto;
  border-radius: 8px;
  margin: 16px 0;
...
.docx-content :deep(.docx-editor img.ProseMirror-selectednode) {
  outline: 3px solid #1890ff;
}

.docx-content :deep(.docx-editor figure) {
  margin: 16px 0;
  text-align: center;
}

.docx-content :deep(.docx-editor figure img) {
  margin: 0;
}

.docx-content :deep(.docx-editor figcaption) {
  margin-top: 8px;
  font-size: 14px;
  color: #8c8c8c;
  font-style: italic;
...
/* 标题样式 */
.docx-content :deep(.docx-editor h1) {
  font-size: 2em;
  font-weight: 700;
  margin: 0.67em 0;
...
.docx-content :deep(.docx-editor h2) {
  font-size: 1.5em;
  font-weight: 600;
  margin: 0.83em 0;
  padding-bottom: 0.3em;
...
.docx-content :deep(.docx-editor h3) {
  font-size: 1.25em;
  font-weight: 600;
  margin: 1em 0;
}

.docx-content :deep(.docx-editor h4),
.docx-content :deep(.docx-editor h5),
.docx-content :deep(.docx-editor h6) {
  font-weight: 600;
  margin: 1em 0;
}

/* 列表样式 */
.docx-content :deep(.docx-editor ul),
.docx-content :deep(.docx-editor ol) {
  padding-left: 1.5em;
  margin: 0.5em 0;
}

.docx-content :deep(.docx-editor li) {
  margin: 0.3em 0;
}

/* 任务列表样式 */
.docx-content :deep(.docx-editor ul[data-type="taskList"]) {
  list-style: none;
  padding-left: 0;
}

.docx-content :deep(.docx-editor ul[data-type="taskList"] li) {
  display: flex;
  align-items: flex-start;
  gap: 8px;
}

.docx-content :deep(.docx-editor ul[data-type="taskList"] li > label) {
  flex: 0 0 auto;
  margin-right: 0.5rem;
  user-select: none;
}

.docx-content :deep(.docx-editor ul[data-type="taskList"] li > label input[type="checkbox"]) {
  cursor: pointer;
  width: 16px;
  height: 16px;
  accent-color: #1890ff;
...
.docx-content :deep(.docx-editor ul[data-type="taskList"] li > div) {
  flex: 1 1 auto;
}

.docx-content :deep(.docx-editor ul[data-type="taskList"] li[data-checked="true"] > div) {
  text-decoration: line-through;
  color: #8c8c8c;
}

/* 代码块样式 - GitHub 风格 */
.docx-content :deep(.docx-editor pre) {
  background: #24292e;
  color: #f6f8fa;
  border-radius: 6px;
  padding: 16px;
...
.docx-content :deep(.docx-editor pre code) {
  background: none;
  padding: 0;
  color: inherit;
  font-size: inherit;
...
/* 行内代码样式 */
.docx-content :deep(.docx-editor code) {
...
/* 引用块样式 */
.docx-content :deep(.docx-editor blockquote) {
...
.docx-content :deep(.docx-editor blockquote p) {
  margin: 0;
}

/* 高亮样式 */
.docx-content :deep(.docx-editor mark) {
...
/* 粗体、斜体、删除线、下划线 */
.docx-content :deep(.docx-editor strong) {
  font-weight: 600;
}

.docx-content :deep(.docx-editor em) {
  font-style: italic;
}

.docx-content :deep(.docx-editor s) {
  text-decoration: line-through;
  color: #6a737d;
}

.docx-content :deep(.docx-editor u) {
  text-decoration: underline;
}

/* 水平线 */
.docx-content :deep(.docx-editor hr) {
  border: none;
...
/* 链接样式 */
.docx-content :deep(.docx-editor a) {
  color: #1890ff;
  text-decoration: none;
}

.docx-content :deep(.docx-editor a:hover) {
  text-decoration: underline;
}

/* 段落间距 */
.docx-content :deep(.docx-editor p) {
  margin: 0.5em 0;
}

.docx-content :deep(.docx-editor p:first-child) {
  margin-top: 0;
}

.docx-content :deep(.docx-editor p:last-child) {
  margin-bottom: 0;
}
